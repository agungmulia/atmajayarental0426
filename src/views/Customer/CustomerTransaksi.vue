...
<PageComponent>
    ...
    <div class="flex justify-between items-center">
      <div class=" lg:w-[50%]">
        ...
        <span class="  text-sm text-pink-600">Perhatian, untuk update disarankan minimal 3 jam sebelum peminjaman, ketika transaksi
          berlangsung anda tidak dapat melakukan perubahan.
        </span>
      </div>
    </div>
  </template>
      ...
      <div class="min-my-3 min-mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8"> 
          <div class="w-[50%] lg:w-full">
            <span class="text-xs text-pink-600">Saat transaksi berjalan anda dapat melakukan pembayaran dengan menekan tombol bayar 
              (Akan tampil ketika status transaksi sedang berjalan)
              ...
              <tbody class="divide-y divide-gray-200 bg-white">       
                ...
                :key="value.ID_TRANSAKSI">
                  ...
                  <td class="w-[20%] px-3 py-4 text-sm text-gray-500">
                      ...
                      <div v-if="value.KETERANGAN_PROMO == null">
                        <span class=" text-base">Tidak Menggunakan Promo</span>
                      </div>
                      <div class=" text-xs">
                  ...
                  <td v-if="value.STATUS_TRANSAKSI == 'Valid'" class=" whitespace-nowrap py-4 pl-3 pr-4 text-sm font-medium sm:pr-6">
                    <div>
                      <router-link 
                      class="text-pink-600 hover:text-pink-900 "
                      :to="{name:'EditTransaksi',params:{id:value.ID_TRANSAKSI}}">
                      Edit</router-link>
                    </div>
                    
                  </td>

                  <td v-if="value.STATUS_TRANSAKSI == 'Sedang berjalan'" class=" whitespace-nowrap py-4 pl-3 pr-4 text-sm font-medium sm:pr-6">
                    <div>
                      <router-link 
                      class="text-pink-600 hover:text-pink-900 text-xl"
                      :to="{name:'Pembayaran',params:{id:value.ID_TRANSAKSI}}">
                      Bayar</router-link>
                    </div>
                  </td>

                  <td v-if="value.STATUS_TRANSAKSI == 'Selesai'" class=" whitespace-nowrap py-4 pl-3 pr-4 text-sm font-medium sm:pr-6">
                    <div>
                      <span
                      class="text-pink-600 hover:text-pink-900 text-xl">
                      Selesai</span>
                    ...
                    <div>
                      ...
                      <router-link 
                      class="bg-pink-600 hover:bg-pink-900 text-xs px-2 py-1 rounded-lg text-white"
                      :to="{name:'CetakNota',params:{id:value.ID_TRANSAKSI}}">
                      Cetak Nota</router-link>
                    </div>
                  </td>
                </tr>
              </tbody>
...
let emailUser =  roleTes;

const numberFormatter = Intl.NumberFormat('en-US');

store.dispatch("getUser").then(() => {
    store.dispatch('getTransaksiByCustomer',emailUser.data.email);
    console.log(emailUser.data.email);
  });
