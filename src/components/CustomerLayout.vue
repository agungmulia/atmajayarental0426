    ...
    <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
      ...
      <DisclosurePanel class="md:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          ...
          active-class="bg-gray-900 text-white" :class="[this.$route.name === item.to.name ? '' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block px-3 py-2 rounded-md text-base font-medium']">{{ item.name }}</router-link>
        </div>
        <div class="pt-4 pb-3 border-t border-gray-700">
          <div class="flex items-center px-5">
            <div class="flex-shrink-0">
              <img v-if="dataCustomer[0].FOTO_CUSTOMER != null" class="h-10 w-10 rounded-full" :src="dataCustomer[0].FOTO_CUSTOMER " alt="" />
            </div>
            <div class="ml-3">
              <div class="text-base font-medium leading-none text-white">{{ dataCustomer[0].NAMA_CUSTOMER }}</div>
              <div class="text-sm font-medium leading-none text-gray-400">{{ dataCustomer[0].EMAIL_CUSTOMER }}</div>
            </div>
          </div>
...
const navigation = [
  { name: 'Dashboard', to: {name: 'Customer'}},
  { name: 'Transaksi', to: {name: 'Transaksi'} },
]
